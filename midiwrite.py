...
def write_midi(pattern):
  ...
  for frame in pattern:
    for i in range(len(frame)):
      if frame[i] == 1:
        m.append(midi.NoteOnEvent(tick=time, channel=1, data=[i+20, 100]))


    time += delta

  m.append(midi.EndOfTrackEvent(tick=time, data=[]))
  midi.write_midifile(midifile="test.mid", pattern=midi.Pattern(format=0, tick_relative=False, resolution=480, tracks=[midi.Track(events=m)]))
